...
Motion Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- MotionMiner : Generates coins (if wallet is enabled).
...
However, in open source software such as Motion Core, where everyone uses
...
pay attention to for reviewers of Motion Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Motion Core uses tinyformat, which is type safe. Leave them out to avoid confusion
