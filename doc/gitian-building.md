...
*Setup instructions for a Gitian build of Motion Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Motion
...
- [Building Motion Core](#building-motion-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-motion-host/README.md).
...
Clone the git repositories for Motion Core and Gitian.
...
in the Motion Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Motion Core
----------------

To build Motion Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Motion Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/motion/.git/
...
For example:
```bash
URL=https://github.com/crowning-/motion.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit motion=${COMMIT} --url motion=${URL} ../motion/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit motion=${COMMIT} --url motion=${URL} ../motion/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit motion=${COMMIT} --url motion=${URL} ../motion/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the motion git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../motion/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/motion-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/motion-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/motion-osx-build.assert
```
