...
###   This script attempts to download the signature file SHA256SUMS.asc from motion.org
...
WORKINGDIR="/tmp/motion"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://motion.org/bin/"
VERSIONPREFIX="motion-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'motion-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Motion Core binary release signing key installed?"
   fi
