...
const char * const MOTION_CONF_FILENAME = "motion.conf";
const char * const MOTION_PID_FILENAME = "motiond.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "motion" is a composite category enabling all Motion-related debug output
            if(ptrCategory->count(string("motion"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MotionCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MotionCore
    // Mac: ~/Library/Application Support/MotionCore
    // Unix: ~/.motioncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MotionCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MotionCore";
#else
    // Unix
    return pathRet / ".motioncore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", MOTION_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty motion.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override motion.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", MOTION_PID_FILENAME));
